...
#define Button_Cap_Select      P0_2
#define Button_Resistor_Select P0_3
#define LCD_RS P2_2
...
void main (void)
{
  ...
  float Capacitance;
    ...
    if(Button_Cap_Select == 1 && Button_Resistor_Select == 1){
			LCDprint("P0_2 for C",1, 1);
			LCDprint("P0_3 for R", 2, 1);
		}

    if(Button_Cap_Select == 0 && Button_Resistor_Select == 1){
			Capacitance = (1440000000*period)/(RA+2*RB);

			sprintf(periodstring, "T = %f s", period);
	 ...
	 if(Button_Resistor_Select == 0 && Button_Cap_Select == 1){
		  Resistance=((period*144.0/(100.0*(Cap/1000000000.0))-RA)/2.0);

			sprintf(periodstring, "T = %f s", period);
 	  	LCDprint(periodstring, 1, 1);

 	  	sprintf(periodstring, "R = %.2f Ohm", Resistance);
 	    LCDprint(periodstring, 2, 1);
	 }

	 if(Button_Resistor_Select == 0 && Button_Cap_Select == 0) {
		 Clean_LCD();
		 LCDprint("Error: Reselect", 1, 1);
		 waitms(100);
	 }
  }
}
